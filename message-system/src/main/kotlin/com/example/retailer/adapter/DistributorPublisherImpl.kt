...
class DistributorPublisherImpl: DistributorPublisher {
    ...
    override fun placeOrder(order: Order): Boolean {
        ...
        return if (order.id != null) {
            template.convertAndSend(topic.name, "distributor.placeOrder.IskandarNabiev.${order.id}", massage) {
                it.messageProperties.headers["Notify-Exchange"] = "distributor_exchange"
                it.messageProperties.headers["Notify-RoutingKey"] = "retailer.IskandarNabiev"
                it
            }
            true
        } else false
